...
import { TrendingUp, Calendar, Tag, Globe, ChevronDown, ChevronUp } from 'lucide-react';
...
export const TopKeywordsChart = ({ articles }: { articles: ArticleData[] }) => {
  ...
  // Filter keywords to show only relevant ones (minimum 2 occurrences and limit to top 50)
  const relevantKeywords = Object.entries(keywordData)
    .filter(([keyword, count]) => count >= 2) // Only keywords that appear at least 2 times
    .filter(([keyword, count]) => {
      // Remove "gift" and "gifting" keywords
      const keywordLower = keyword.toLowerCase();
      return keywordLower !== 'gift' && keywordLower !== 'gifting';
    })
    .sort((a, b) => b[1] - a[1])
    .slice(0, 50) // Limit to top 50 most relevant keywords
    .map(([keyword, count]) => ({
      keyword,
      count
    }));

  const keywordChartData = showAllKeywords 
    ? relevantKeywords 
    : relevantKeywords.slice(0, 15);

  const hasMoreKeywords = relevantKeywords.length > 15;
  ...
  return (
    <Card className="border-none shadow-md hover:shadow-lg transition-shadow duration-200">
      <CardHeader className="space-y-1">
        ...
        <CardDescription>
          Most frequently occurring keywords in articles
          {hasMoreKeywords && (
            <span className="ml-2 text-xs text-muted-foreground">
              ({relevantKeywords.length} total keywords)
            </span>
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-4">
        <ResponsiveContainer width="100%" height={showAllKeywords ? 600 : 400}>
          <BarChart data={keywordChartData} layout="vertical" margin={{ left: 120, right: 20, top: 20, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted))" opacity={0.3} />
            <XAxis
              type="number"
              tick={{ fontSize: 12, fill: 'hsl(var(--foreground))' }}
            ...
            <YAxis
              ...
              stroke="hsl(var(--muted-foreground))"
              tickFormatter={(value) => {
                // Show full keyword name, truncate only if very long
            ...
            >
              {showAllKeywords ? (
                ...
                <>
                  ...
                  Show All Keywords ({relevantKeywords.length})
                </>
              )}
            </Button>
          </div>
